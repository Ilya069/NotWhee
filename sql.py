		...
		with conn.cursor() as cursor:
			...
			if seconds >= 85:
				...
				if textchoice != 'None':
					...
					maxbet += bet
					if (maxbet <= 300000000) and (bet > 99):
						...
						cursor.execute(F"UPDATE goldenusers SET last_choice = 'None',last_number = 'None' WHERE userid = '{userid}'")
						message = F"{name}, максимальная сумма ставок: 300 000 000 коинов, минимальная: 100."
						messages_send(roomid,message,menu.main())
	except Exception:
		traceback.print_exc()
...
def get_top():
	try:
		...
		with conn.cursor() as cursor:
			...
			number = 0
			message = "ТОП 10 игроков (конкурс, подробности на стене)\n\n"
			for i in users:
				userid = i[0]
				name = i[1]
				wins_all = int(i[2])
